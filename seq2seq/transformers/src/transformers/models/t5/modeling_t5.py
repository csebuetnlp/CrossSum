    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if lang_ids is not None:
                langid_embeddings = self.embed_tokens(lang_ids)
                langid_embeddings = torch.stack(
                    [langid_embeddings[i].repeat(input_shape[1], 1) for i in range(langid_embeddings.shape[0])]
                ) # (batch_size, seq_length, dim)

                if self.is_decoder:
                    langid_embeddings[..., 0, :] = 0

                inputs_embeds += langid_embeddings
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        add_langid_embeddings = (
            self.config.task_specific_params.get("add_langid_embeddings", "") if 
                self.config.task_specific_params is not None else ""
        )
        lang_ids = None
        if decoder_input_ids is not None:
            lang_ids = decoder_input_ids[:, 0] # (batch_size, 1)
        ...
        decoder_outputs = self.decoder(
            ...
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
            lang_ids=lang_ids if add_langid_embeddings == "decoder" else None,
        )
